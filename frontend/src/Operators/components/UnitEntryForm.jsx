...
export function UnitEntryForm({
    ...
    // 🔑 FOCUS MANAGEMENT LOGIC (FIXED FOR 1-TAP TYPING)
    useEffect(() => {
    ...
    // 🔑 NEW LOGIC: Clear board serials when "No Good (NG)" is selected
    const handleStatusChange = (e) => {
        const newStatus = e.target.value;
        let updatedData = { ...formData, status: newStatus };

        if (newStatus === "No Good (NG)") {
            // Clear all board serial fields in formData
    ...
    const handleFormSubmit = (e) => {
        ...
        // VALIDATION: Requires Remarks if status is "No Good (NG)"
        ...
        // VALIDATION: Requires all 5 PCBA serials if status is "In Progress" at Station 1
        ...
        // Existing Station 1 general check
        if (isStation1) {
            const missing = PCBA_FIXED_DATA.filter(b => !formData[b.key] || formData[b.key].trim().length < 6);
            ...
            <div className="col-12">
                <div className={`card border-0 shadow-sm ${isScannerEnabled ? 'bg-white' : 'bg-light'}`} 
                    ...
                    <div className="card-body py-2 px-3 d-flex justify-content-between align-items-center">
                        ...
                        <div className="form-check form-switch"><input className="form-check-input" type="checkbox" checked={isScannerEnabled} onChange={(e) => setIsScannerEnabled(e.target.checked)} /></div>
                    </div>
                ...
                <form onSubmit={handleFormSubmit} className="card border-0 shadow-sm" style={{ borderRadius: '8px', overflow: 'hidden' }}>
                    <div className="card-header bg-white border-bottom py-3 px-4">
                        <div className="d-flex justify-content-between align-items-center">
                            ...
                            <div className="text-end" style={{ width: '120px' }}>
                                <div className="progress" style={{ height: '4px', borderRadius: '0' }}><div className="progress-bar bg-success" style={{ width: `${progressPercent}%` }}></div></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="card-body p-4 bg-light-subtle">
                        <div className="row g-3 mb-4">
                            <div className="col-md-4"><label className="form-label text-muted fw-bold small mb-1">MODEL</label><input type="text" className="form-control form-control-sm border-0 bg-light" value={formData.model || ''} readOnly /></div>
                            <div className="col-md-4"><label className="form-label text-muted fw-bold small mb-1">REVISION</label><input type="text" className="form-control form-control-sm border-0 bg-light" value={formData.revision || ''} readOnly /></div>
                            <div className="col-md-4"><label className="form-label fw-bold small mb-1">ASSEMBLY NO.</label><input type="text" className="form-control form-control-sm border-primary bg-white fw-bold shadow-sm" value={formData.assemblyNo || ''} readOnly placeholder="SCAN QR CODE" /></div>
                        </div>
                        ...
                        {isStation1 && (
                            <div className="col-12 mb-4 animate-in slide-in">
                                <div className="p-3 border rounded bg-white shadow-sm" style={{ borderTop: '4px solid #0d6efd' }}>
                                    <h6 className="fw-bold text-primary mb-3 small"><i className="bi bi-cpu-fill me-2"></i>PCBA BOARD DETAILS (REQUIRED FOR PAIRING)</h6>
                                        ...
                                        <table className="table table-sm table-bordered align-middle text-center small" style={{ fontSize: '0.7rem' }}>
                                            <thead className="table-light">
                                                <tr>
                                                    <th>PCBA MODEL</th>
                                                    <th>PARTS CODE</th>
                                                    <th className="bg-primary-subtle text-primary">BOARD SERIAL (Huling 6 na Digit) {formData.status === 'In Progress' && <span className="text-danger">*</span>}</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                        ...
                                                        <td style={{ width: '280px' }}>
                                                            <div className="input-group input-group-sm">
                                                                ...
                                                                <input 
                                                                    ...
                                                                    className={`form-control form-control-sm fw-bold ${statusMessage && statusMessage.includes(formData[board.key]) && formData[board.key] !== "" ? 'border-danger text-danger bg-danger-subtle' : 'border-primary text-primary'}`} 
                                                                    placeholder="000000"
                        ...
                        <div className="row g-3">
                            <div className="col-md-6"><label className="form-label text-muted fw-bold small mb-1">SERIAL NO.</label><input type="text" className="form-control form-control-sm bg-light" value={formData.deviceSerialNo || ''} readOnly /></div>
                            <div className="col-md-6"><label className="form-label text-muted fw-bold small mb-1">ACCESSORY NO.</label><input type="text" className="form-control form-control-sm bg-light" value={formData.accessoryKittingNo || ''} readOnly /></div>
                            <div className="col-12 mt-3"><label className="form-label fw-bold small mb-1">STATUS <span className="text-danger">*</span></label>
                                <select className="form-select form-select-sm border-secondary-subtle" value={formData.status || 'In Progress'} onChange={handleStatusChange}>
                            ...
                            <div className="col-12">
                                ...
                                <textarea className={`form-control form-control-sm ${formData.status === 'No Good (NG)' ? 'border-danger' : 'border-secondary-subtle'}`} rows="2" placeholder={formData.status === 'No Good (NG)' ? "Please describe the defect..." : "Notes for this unit..."} value={formData.remarks || ''} onChange={(e) => setFormData({...formData, remarks: e.target.value})}></textarea>
                            </div>
                        </div>
